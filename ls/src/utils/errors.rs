...
impl Errors {
  fn get_error_message(&self, file_name: String) -> String {
    ...
    let file_name_meta = String::from("[Meta] You need to specify a file name for this error type");

    match self {
      Errors::CorruptFile => { format!("{}!", corrupt_file) },
      Errors::MetadataFailure => { format!("{}! File name: {}", metadata_failure, file_name)},
      Errors::RowFailure => { format!("{}! File name: {}", row_failure, file_name)},
  ...
  let file = match file_name {
    ...
    None => {
      if err_type == Errors::RowFailure {
        panic!("{}", Errors::FileNameMeta.get_error_message(String::from("")))
      } else {
        String::from("")
      }
    }
